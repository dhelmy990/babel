...
TEST_CASE("dashboard forwards one isolated smoke rerun through worker") {
  Repository repository;
  ...
  CHECK(performance_worker.prepared->matrix == "isolated-smoke");
  CHECK(body(response).at("trial").at("operatorApproved") == false);
}

TEST_CASE("dashboard rejects an unknown representative matrix selector") {
  Repository repository;
  repository.performance.population_ready = true;
  repository.performance.run_id = ExperimentRunId::parse(
      "55555555-5555-5555-8555-555555555555").value();
  ...
  CHECK_FALSE(performance_worker.prepared.has_value());
}
