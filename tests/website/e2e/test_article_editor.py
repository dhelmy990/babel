...
from tests.website.e2e.conftest import PNG
...
def test_edit_existing_body_and_retain_image(editable_owner_page, live_server):
    ...
    expect(page.get_by_label("Import Markdown", exact=True)).to_have_count(1)
    expect(page.locator(".existing-images, [data-insert-image]")).to_have_count(0)
