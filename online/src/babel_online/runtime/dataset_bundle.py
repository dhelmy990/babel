...
SCALE_DATASET_CONFIG = "crosswalk_2026_06_07"
SCALE_DATASET_REVISION = "0d1ab2c7f0e2295682288fcf10077d2d776bf559"
SCALE_REQUIRED_CONFIGS = frozenset(
    {
        "catalog_2026_06",
...
def acquire_pinned_bundle(
    *,
    ...
    if repo_id != DEMO_DATASET_REPOSITORY or not token:
        raise DatasetBundleIntegrityError("private dataset requires its exact revision and token")
    if revision == DEMO_DATASET_REVISION:
        allow_patterns = [f"{name}/**" for name in sorted(REQUIRED_CONFIGS)]
    elif revision == SCALE_DATASET_REVISION:
        allow_patterns = [f"{name}/**" for name in sorted(SCALE_REQUIRED_CONFIGS)]
        allow_patterns.append("monthly-engineering-snapshots/release-manifest.json")
    else:
        raise DatasetBundleIntegrityError(
            "private dataset requires an accepted exact revision"
        )
...
def load_scale_dataset_bundle(
    ...
    *,
    ...
    """Load the real time-boxed June/July engineering snapshot."""
    root_path = Path(root).resolve()
    if (
        dataset_repository != DEMO_DATASET_REPOSITORY
        or dataset_config != SCALE_DATASET_CONFIG
        or dataset_revision != SCALE_DATASET_REVISION
    ):
        raise DatasetBundleIntegrityError("dataset identity is not the pinned scale bundle")
    manifest_path = (
        root_path / "monthly-engineering-snapshots" / "release-manifest.json"
    )
    try:
        manifest_bytes = manifest_path.read_bytes()
        manifest = json.loads(manifest_bytes)
        ...
        raise DatasetBundleIntegrityError(
            "scale dataset release manifest is unavailable"
        ) from error
    configs = manifest.get("configs")
    if (
        manifest.get("schema_version") != 1
        or manifest.get("release_scope") != "timeboxed_engineering_snapshot"
        or not isinstance(configs, dict)
        or set(configs) != SCALE_REQUIRED_CONFIGS
    ):
        raise DatasetBundleIntegrityError(
            "scale release must bind both real months and their crosswalk"
        )
    loaded: dict[str, tuple[dict[str, Any], ...]] = {}
