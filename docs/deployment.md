...
[`test_other_users_including_publisher_cannot_access_notes`](../tests/test_notes.py),
[`test_archive_permissions_and_access_grant`](../tests/test_archive.py),
and reader-switch/browser review checks in [`tests/e2e/test_reviews.py`](../tests/e2e/test_reviews.py).
...
STUDY_AGE_BINARY='/REPLACE_WITH_VERIFIED_LOCAL_AGE_BINARY' \
  .venv/bin/python -m pytest tests/test_deployment.py tests/test_smoke.py tests/test_recovery_bundle.py -q
```
