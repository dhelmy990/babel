...
For production deployment, recovery, DNS, OAuth, email, and encrypted off-VM backup procedures, see [the operator runbook](deployment.md).
...
docker compose -f compose.website.yaml --env-file /dev/null up -d --wait db
...
Open http://127.0.0.1:8000/. `npm run start:website` calls the virtual environment's Python
...
```bash
source .venv/bin/activate
pytest -c pytest.website.ini tests/website/test_pages.py -q
pytest -c pytest.website.ini -q
...
```bash
source .venv/bin/activate
python -m pytest -c pytest.website.ini -q
python manage.py makemigrations --check --dry-run
docker build -f Dockerfile.website --build-arg STUDY_RELEASE="$(git rev-parse HEAD)" -t study-d1:verification .
STUDY_DEPLOYMENT_IMAGE=study-d1:verification python -m pytest -c pytest.website.ini tests/website/test_deployment.py -q
